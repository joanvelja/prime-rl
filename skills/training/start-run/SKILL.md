...
- For text-only multimodal-family runs such as Gemma4, set `inference.language_model_only=true` so vLLM serves the language path intentionally.
- For Gemma4 26B-A4B MoE at `seq_len=2048` on GH200, verify candidate nodes with `nvidia-smi` before launch. A node with persistent GPU memory use can fail vLLM startup with an insufficient KV-cache error even when other nodes are healthy.
- If KV cache is just short on a clean GH200 node, try a modest `gpu_memory_utilization` increase in the config. In the 2026-05-28 stress run, `0.78` was enough for `google/gemma-4-26B-A4B-it` on clean nodes.
- Keep vLLM warnings visible. Treat default MoE/LoRA kernel-config warnings as performance signals, not errors to suppress.
