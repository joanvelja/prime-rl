...
class ActivationCheckpointConfig(BaseConfig):
    ...
    targets: Annotated[
        list[str],
        Field(
            description="Selective checkpoint targets. `norm` checkpoints every norm module inside selected layers (decoder, attention, MLA, etc.). `attn_proj` checkpoints projection-side attention work outside the kernel, including input/output projections, attention-local norms, RoPE, gating, and model-specific MLA projection helpers where exposed. `mlp` checkpoints the entire dense MLP forward (not applicable to MoE layers). `mla_up_proj` checkpoints MLA Q/KV up-projection work where supported. `moe_act` checkpoints the routed-expert activation inside MoE layers, including SwiGLU in `MoE` and relu2 in `LatentMoE`. `routed_experts` checkpoints the broader routed expert path in MoE layers (including LatentMoE), so it subsumes `moe_act`. `linear_attn` checkpoints supported token mixers outside the standard softmax-attention path, including NemotronH Mamba layers, Qwen3.5-MoE GatedDeltaNet layers, and AFMoE sliding-window attention layers.",
        ),
    ] = ["norm"]
