...
class EnvArgsSchedulerConfig(BaseModel):
    """Config for an external environment args scheduler."""

    import_path: Annotated[
        str,
        Field(description="Import path to the args scheduler function (e.g. 'my_module.schedule_env_args')"),
    ]
    kwargs: Annotated[
        dict[str, Any],
        Field(default_factory=dict, description="Kwargs to pass to the args scheduler function"),
    ]
    interval: Annotated[
        int,
        Field(ge=1, description="How often to call the args scheduler, in orchestrator steps."),
    ] = 1
...
class EnvConfig(BaseConfig):
    ...
    args_scheduler: Annotated[
        EnvArgsSchedulerConfig | None,
        Field(description="Optional scheduler that can update env args over training."),
    ] = None
