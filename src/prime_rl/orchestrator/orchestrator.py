        ...
        def prepare_candidate_batch(
            ...
            if is_ma:
                ...
                for rollout, unit_idxs in zip(candidate_rollouts, candidate_rollout_to_unit_idxs):
                    ...
                    # Persist per-member advantage AND reward so the dumped JSONL
                    # is self-describing: the episode-mean ``advantage`` above is
                    # ~0 by zero-sum cancellation in self-play, which hides the
                    # per-seat credit. Keyed by member_id (MemberRollout top-level
                    # field). Dump-only — does not feed training.
                    rollout["member_advantages"] = {
                        candidate_training_units[i]["member_id"]: candidate_training_units[i]["advantage"]
                        for i in unit_idxs
                    }
                    rollout["member_rewards"] = {
                        candidate_training_units[i]["member_id"]: candidate_training_units[i]["reward"]
                        for i in unit_idxs
                    }
