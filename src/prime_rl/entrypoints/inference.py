...
def write_slurm_script(config: InferenceConfig, config_path: Path, script_path: Path) -> None:
    ...
    if is_disaggregated:
        template_vars.update(
            ...
            kv_offload_block_size=config.deployment.kv_cache_offload.block_size if config.deployment.kv_cache_offload else 64,
            kv_offload_cpu_bytes=int(config.deployment.kv_cache_offload.cpu_bytes) if config.deployment.kv_cache_offload else 0,
        )
    elif is_multi_node:
        template_vars.update(
            router_port=config.deployment.router_port,
