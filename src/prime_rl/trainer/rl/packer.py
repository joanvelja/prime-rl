...
class SinglePacker(BasePacker):
    def __init__(
        ...
        super().__init__(dp_world_size, seq_len, pad_to_multiple_of, tokenizer, config, start_step, pack_samples)
...
def setup_packer(
    ...
    if multi_run_manager.max_runs == 1:
        return SinglePacker(
            dp_world_size, seq_len, pad_to_multiple_of, tokenizer, transport_config, start_step, pack_samples
        )
    else:
        return MultiPacker(
            dp_world_size, seq_len, pad_to_multiple_of, tokenizer, transport_config, start_step, pack_samples
