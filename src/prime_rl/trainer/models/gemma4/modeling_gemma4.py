...
# ---------------------------------------------------------------------------
# MoE Experts (w1/w2/w3 format with gelu_pytorch_tanh)
# ---------------------------------------------------------------------------


_GEMMA4_ACT_FN = ACT2FN["gelu_pytorch_tanh"]


def _run_experts_for_loop_impl(w1, w2, w3, x, num_tokens_per_expert):
    ...
    for expert_idx, x_expert in enumerate(x_splits):
        h = _GEMMA4_ACT_FN(torch.matmul(x_expert, w1[expert_idx].transpose(-2, -1)))
...
def _run_experts_grouped_mm_impl(w1, w2, w3, x, num_tokens_per_expert):
    offsets = torch.cumsum(num_tokens_per_expert, dim=0, dtype=torch.int32)
    h = _GEMMA4_ACT_FN(torch._grouped_mm(x.bfloat16(), w1.bfloat16().transpose(-2, -1), offs=offsets))
...
class Gemma4Experts(GroupedExperts):
    """Gemma4 experts — inherits EP dispatch from GroupedExperts, overrides activation.

    Uses @expert_parallel decorated functions that handle DTensor→local conversion
    and token permutation for EP.
    """
...
class Gemma4ParallelMoE(MoE):
    """Gemma4 parallel MoE: shared dense MLP + sparse experts run in parallel.

    Inherits from MoE so the framework's EP/FSDP discovery (isinstance checks)
    works without any changes to model.py.
    """

    def __init__(self, config: Gemma4TextConfig):
        nn.Module.__init__(self)  # skip MoE.__init__
        self.shared_mlp = Gemma4MLP(config)
        self.router = Gemma4Router(config)
        self.experts = Gemma4Experts(config, use_grouped_mm=getattr(config, "use_grouped_mm", True))
        self.register_buffer(
            "tokens_per_expert", torch.zeros(config.num_experts, dtype=torch.float32), persistent=False
        )

    def set_ep_comm_backend(self, backend: str) -> None:
        self.experts.set_ep_comm_backend(backend)

    def set_deepep_token_chunk_size(self, chunk_size: int | None) -> None:
        pass  # Gemma4 uses torch EP only
    ...
    def forward(
        ...
        # Shared MLP branch (normed input)
        shared_out = self.shared_mlp(pre_feedforward_layernorm(hidden_states))
        shared_out = post_feedforward_layernorm_1(shared_out)

        # Sparse expert branch (un-normed input for router, separately normed for experts)
        ...
        expert_input = pre_feedforward_layernorm_2(hidden_states_flat)
        ...
        expert_out = torch.zeros_like(hidden_states_flat)
        expert_out.scatter_add_(dim=0, index=routed_indices, src=routed_output)
        expert_out = expert_out.reshape(hidden_states.shape)
...
class Gemma4MoeDecoderLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config: Gemma4TextConfig, layer_idx: int):
        ...
        self.mlp = Gemma4ParallelMoE(config)
...
class Gemma4PreTrainedModel(PreTrainedModelPrimeRL):
    ...
    def convert_to_hf(cls, state_dict: dict[str, Tensor]) -> dict[str, Tensor]:
        from prime_rl.trainer.models.gemma4.converting_gemma4 import convert_prime_to_hf
    ...
    def convert_layer_to_hf(cls, state_dict: dict[str, Tensor], layer_idx: int) -> dict[str, Tensor]:
        from prime_rl.trainer.models.gemma4.converting_gemma4 import convert_prime_layer_to_hf
        ...
        **kwargs,
        ...
        if pixel_values is not None:
            ...
            vision_output = self.vision_tower(
                pixel_values, pixel_position_ids=image_position_ids, return_dict=True
            )
...
class Gemma4ForCausalLM(Gemma4PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, **kwargs):
        ...
        if self._is_vlm:
            # VLM mode: keep composite config, create vision tower + language model
            ...
            # Text-only mode: use text config directly
            super().__init__(config, **kwargs)
