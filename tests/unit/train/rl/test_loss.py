...
def test_default_loss_clips_importance_ratio():
    inputs = LossInputs(
        trainer_logprobs=torch.tensor([2.0, 0.0], dtype=torch.float32).cuda(),
    ...
    unclipped = setup_loss_fns(DefaultLossConfig(dppo_mask_high=10.0))["rl"](inputs)
    clipped = setup_loss_fns(DefaultLossConfig(dppo_mask_high=10.0, importance_ratio_clip=1.0))["rl"](inputs)

    assert clipped.loss > unclipped.loss
    assert torch.isclose(clipped.metrics["importance_ratio"], torch.tensor(1.0, device=clipped.loss.device))
    assert clipped.metrics["importance_ratio_raw"] > clipped.metrics["importance_ratio"]
    assert clipped.metrics["importance_ratio_clipped"] > 0
