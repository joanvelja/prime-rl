"""Tests for Qwen3 Renderer — every case compares against tokenizer.apply_chat_template()."""

import pytest
from transformers import AutoTokenizer

from prime_rl.rendering import Qwen3Renderer, build_supervised_sample, build_trajectory_step
...
def test_generation_prompt_thinking_disabled(tokenizer):
    renderer = Qwen3Renderer(tokenizer, enable_thinking=False)
    msgs = [{"role": "user", "content": "Hi"}]
    assert renderer.render_ids(msgs, add_generation_prompt=True) == _expected(
...
def test_thinking_before_last_query(tokenizer, renderer):
    msgs = [
        {"role": "user", "content": "Hi"},
...
def test_tool_call_no_content(tokenizer, renderer):
    msgs = [
        {"role": "user", "content": "Weather?"},
        {
            "role": "assistant",
